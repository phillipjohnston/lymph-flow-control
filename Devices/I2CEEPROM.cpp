...
void I2CEEPROM::eepromwrite(){
  ...
  while(Serial.available() == 0){ // Both LEDs go high, wait for user input (USB)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
//This loop reads in serial data, parsing each int by a linefeed, and terminating the loop when it///////
//recieves a 'z'.                                                                                 ///////
/////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  while(in != 'z'){
	if(Serial.available() > 0 ){
		...
		if(in != 10){		// LF not recieved, so this char must be part of an int
			curNum += in;
			decPlace++;
		}
...
int I2CEEPROM::getFreq(){   	// Getter for motor update frequency variable
	Wire.begin();
...
int I2CEEPROM::getDuration(){  // Getter for experiment duration variable
	Wire.begin();
...
int I2CEEPROM::getSamples(){  // Getter for the number of samples needed to read
	int samples = x-2;
	return samples;
}

int I2CEEPROM::getMotorVal(int i){ // Getter for an individual motor value
